...
function systemPrompt(node: FlowNode, priorAnswer?: string): string {
  return [
    ...
    'DEFAULT TO ENGAGED: as long as the person keeps replying, treat them as interested. Only pick a negative/disqualifying intent on an unmistakable, explicit signal. "not_interested" means ONLY a clear brush-off ("not interested", "no thanks", "stop messaging me", "leave me alone"). A short, casual, playful, curious, agreeing, or positive-but-vague reply (e.g. "more is always possible right? :)", "yeah maybe", "sure", "why do you ask?", "haha true") is engagement, NOT rejection: use "agree", "answer", "content_question", or "other". When unsure between a positive and a negative intent, always choose the positive one.',
